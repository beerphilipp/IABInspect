...
public class MySetupApplication implements ITaintWrapperDataFlowAnalysis {
    ...
    protected MySetupApplication.IInPlaceInfoflow infoflow = null;
     ...
     *         callback analysis
     */
    protected FlowDroidTimeoutWatcher createCallbackTimeoutWatcher(final InfoflowAndroidConfiguration.CallbackConfiguration callbackConfig,
     ...
     *
     */
    protected class InPlaceInfoflow extends Infoflow implements MySetupApplication.IInPlaceInfoflow {
    ...
    protected class InPlaceBackwardsInfoflow extends BackwardsInfoflow implements MySetupApplication.IInPlaceInfoflow {
    ...
    public InfoflowResults runInfoflow(ISourceSinkDefinitionProvider sourcesAndSinks) {
        ...
        MySetupApplication.MultiRunResultAggregator resultAggregator = new MySetupApplication.MultiRunResultAggregator(config.getPathAgnosticResults());
    ...
    protected void processEntryPoint(ISourceSinkDefinitionProvider sourcesAndSinks,
                                     MySetupApplication.MultiRunResultAggregator resultAggregator, int numEntryPoints, SootClass entrypoint) {
     ...
     */
    protected MySetupApplication.IInPlaceInfoflow createInfoflow() {
        // Some sanity checks
        if (config.getSootIntegrationMode().needsToBuildCallgraph()) {
        ...
        MySetupApplication.IInPlaceInfoflow info = createInfoflowInternal(lifecycleMethods);
        ...
        info.addPostProcessor(new PostAnalysisHandler() {
            ...
            public InfoflowResults onResultsAvailable(InfoflowResults results, IInfoflowCFG cfg) {
                ...
                final InfoflowAndroidConfiguration.IccConfiguration iccConfig = config.getIccConfig();
     ...
     */
    protected MySetupApplication.IInPlaceInfoflow createInfoflowInternal(Collection<SootMethod> lifecycleMethods) {
        final String androidJar = config.getAnalysisFileConfig().getAndroidPlatformDir();
        if (config.getDataFlowDirection() == InfoflowConfiguration.DataFlowDirection.Backwards)
            return new MySetupApplication.InPlaceBackwardsInfoflow(androidJar, forceAndroidJar, cfgFactory, lifecycleMethods);
        return new MySetupApplication.InPlaceInfoflow(androidJar, forceAndroidJar, cfgFactory, lifecycleMethods);
    }
