...
class WebViewChromium implements WebViewProvider, WebViewProvider.ScrollDelegate,
    ...
    public static void logCallToSQLite(int api, String args) {
        // API call id to name
        String api_name = "";
        try {
            for (Field f : ApiCall.class.getDeclaredFields()) {
                if (api == f.getInt(null)) {
                    api_name = f.getName();
                    break;
                }
            }
        } catch (IllegalAccessException e) {
            Log.e("WEBVIEW_API_CALL_MAPPING", e.toString());
        }
        // Get APK name from Stacktrace
        String trace = "";
        StackTraceElement[] stackTraceElements = Thread.currentThread().getStackTrace();
        for (StackTraceElement se : stackTraceElements) {
            trace += se.getClassName() + "." + se.getMethodName() + " (" + se.getFileName() + ":" + se.getLineNumber() + ")\\n";
        }
        // Log.i("WEBVIEW_API_CALL", "API CALL: " + api_name);
        // Log.i("WEBVIEW_API_CALL", "ARGUMENTS: " + args);
        // Log.i("WEBVIEW_API_CALL", "TRACE: " + trace);
        String data_path = "/storage/emulated/0/Download/webview_data.raw";
        File file = new File(data_path);
        FileWriter f;
        try {
            f = new FileWriter(file, true);
            f.write(api_name + "$$$$" + args + "$$$$" + trace + "\n€€€€\n");
            f.close();
        } catch (IOException e) {
            Log.e("WEBVIEW_API_CALL_WRITER", e.toString());
        }
        /*
        String sqlite_path = "/storage/emulated/0/Download/data.sqlite";
        try {
            File file = new File(sqlite_path);
            SQLiteDatabase mDB;
            if(file.exists()) {
                mDB = openOrCreateDatabase(sqlite_path, 0x0001, null);
                mDB.execSQL("CREATE TABLE IF NOT EXISTS data (id integer PRIMARY KEY, app VARCHAR(128), sink VARCHAR(128), args TEXT);");
            } else {
                mDB = openDatabase(sqlite_path, 0x0001, null);
            }
            ContentValues contentValues = new ContentValues();
            contentValues.put("app", caller);
            contentValues.put("sink", method);
            contentValues.put("args", args);
            mDB.insert("data", null, contentValues);
        } catch (Exception e) {
            Log.e("WEBVIEW_DATABASE", e.toString());
        }
        */
    }
    ...
    public static void recordWebViewApiCall(int sample, Object[] parameters) {
        ...
        logCallToSQLite(sample, json.toString());
    }
    ...
    public void loadUrl(final String url, final Map<String, String> additionalHttpHeaders) {
        ...
        if (CommandLine.getInstance().hasSwitch(AwSwitches.WEBVIEW_FORCE_WEBSITE)) {
            ...
            tmpUrl = "https://127.0.0.1:8443" //CommandLine.getInstance().getSwitchValue(AwSwitches.WEBVIEW_FORCE_WEBSITE);
        }
            ...
            mFactory.addTask(new Runnable() {
                ...
                public void run() {
                    try (TraceEvent event =
                        ...
                        mAwContents.loadData(newUrl, mimeType, encoding);
                    }
                }
            });
            return;
    ...
    public void loadDataWithBaseURL(final String baseUrl, final String data, final String mimeType,
            ...
            mFactory.addTask(new Runnable() {
                ...
                public void run() {
                    try (TraceEvent event = TraceEvent.scoped(
                        ...
                        recordWebViewApiCall(ApiCall.LOAD_DATA_WITH_BASE_URL, convertToParameterArray(baseUrl, newUrl, mimeType, encoding, historyUrl));
                        recordWebViewApiCall(ApiCall.LOAD_DATA_WITH_BASE_URL);
                        mAwContents.loadDataWithBaseURL(
                                baseUrl, newUrl, mimeType, encoding, historyUrl);
                    }
                }
            });
            return;
        ...
        try (TraceEvent event =
            ...
            recordWebViewApiCall(ApiCall.LOAD_DATA_WITH_BASE_URL);
            mAwContents.loadDataWithBaseURL(baseUrl, newUrl, mimeType, encoding, historyUrl);
        }
    }
    ...
    public int getRendererRequestedPriority() {
        try (TraceEvent event = TraceEvent.scoped(
            ...
            @RendererPriority final int awRendererRequestedPriority = mAwContents.getRendererRequestedPriority();
    ...
    public void onDraw(final Canvas canvas) {
        ...
        try {
            recordWebViewApiCall(ApiCall.DRAW, convertToParameterArray(canvas));
        } catch (Exception e) {
            Log.e("WEBVIEW_API_CALL_CUSTOM_ID", e.toString());
        }
